			...
			<div id="content">
				...
				<header>
					...
					<?php
									  ...
									  // collect tournaments according to parameters
									  $rows = array();
									  if (!empty($_GET['ID'])){
										$single = getTournamentById($_GET['ID']);
										if ($single !== null) $rows[] = $single;
									  } elseif (!empty($_GET['d'])){
										// exact date match
										foreach (load_data()['tournaments'] as $t){
											if (isset($t['StartDate']) && $t['StartDate'] == $_GET['d']) $rows[] = joinTournamentWithLocation($t);
										}
									  } else {
										// default: return all tournaments
										foreach (load_data()['tournaments'] as $t){
											$rows[] = joinTournamentWithLocation($t);
										}
									  }
