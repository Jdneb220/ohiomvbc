				...
				<div class="inner">
					...
					<?php
							...
							<?php
							  ...
							  $tourneyQuery = "SELECT tournaments.Name, tournaments.StartDate from tournaments where StartDate >= '".date("Y-m-d H:i:s"). "' order by StartDate asc";
							  $tourneyQueryResult = mysql_query($tourneyQuery);
								
							  while ($row = mysql_fetch_array($tourneyQueryResult, MYSQL_ASSOC)) { 
								echo "<option value='"	. $row[Name] . " (" . $row[StartDate] . ")'>"  . $row[Name] . " (" . tournamentShortDateFormat($row[StartDate]) . ")</option>";
							  }
							?>
							</select>
							</label>
